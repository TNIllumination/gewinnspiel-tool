...
/// Ein Fehler, den der Betreiber selbst beheben kann. Wird von alsErgebnis
/// in einen Rueckgabewert verwandelt — sonst wirft Next.js den Text weg.
function fail(message: string): never {
  throw new Bedienfehler(message);
}
...
export async function logout() {
  // Ohne alsErgebnis: wird direkt als Formularziel benutzt und darf deshalb
  // nichts zurueckgeben. Fehler kann es hier ohnehin keine geben.
...
export async function previewManualImport(
  raw: string,
): Promise<ImportPreviewResult | MitFehler> {
  return alsErgebnis(async () => {
    await requireUser();
...
async function githubZugang() {
  ...
  if (!repo || !settings?.githubToken) return null;

  let token: string | null;
  try {
    token = decryptOptional(settings.githubToken);
  } catch {
    // Der Schluessel liegt verschluesselt in der Datenbank, der zugehoerige
    // Hauptschluessel in .env. Passen die nicht zusammen, ist die .env eine
    // andere als beim Speichern — etwa nach einer Neuinstallation.
    fail(
      "Der hinterlegte Zugangsschlüssel lässt sich nicht mehr lesen. Das " +
...
export async function buildTexts(giveawayId: string): Promise<TextsResult | MitFehler> {
  return alsErgebnis(async () => {
    await requireUser();
...
export async function testGitHubConnection() {
  return alsErgebnis(async () => {
    ...
    if (!settings?.githubRepo?.trim()) {
      fail("Es ist noch kein Repository eingetragen. Trag es oben ein und speichere.");
    }
    if (!settings?.githubToken) {
      fail(
        "Es ist noch kein Zugangsschlüssel hinterlegt. Trag ihn oben ein und " +
          "speichere — die Anleitung dazu steht direkt darunter.",
      );
    }

    const zugang = await githubZugang();
    if (!zugang) fail("Repository oder Zugangsschlüssel fehlen.");
